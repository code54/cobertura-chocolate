...
public class LineData
    ...
    @ElementList(entry="jumps", inline=true, required = false)
	...
	public boolean equals(Object obj){
		...
		try{
			return (this.hitsCount == lineData.hitsCount)
		...
		try{
			if (jumps != null)
				for (int i = jumps.size() - 1; i >= 0; i--)
					ret += (jumps.get(i)).getNumberOfValidBranches();
			if (switches != null)
				for (int i = switches.size() - 1; i >= 0; i--)
					ret += (switches.get(i)).getNumberOfValidBranches();
			return ret;
		}finally{
			lock.unlock();
		}
	...
	public int getNumberOfCoveredBranches(){
		...
		try{
			if (jumps != null)
				for (int i = jumps.size() - 1; i >= 0; i--)
					ret += (jumps.get(i)).getNumberOfCoveredBranches();
			if (switches != null)
				for (int i = switches.size() - 1; i >= 0; i--)
					ret += (switches.get(i)).getNumberOfCoveredBranches();
			return ret;
		}finally{
			lock.unlock();
		}
	...
	public void merge(CoverageData coverageData){
		...
		getBothLocks(lineData);
		try{
			this.hitsCount += lineData.hitsCount;
	...
	SwitchData getSwitchData(int switchNumber, SwitchData data){
		...
		try{
			...
			return switches.get(switchNumber);
		}finally{
			lock.unlock();
		}
	}
